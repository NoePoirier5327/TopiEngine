...
namespace topi::command {
  ...
  class CommandManager {
    public:
       ...
       */
      void on_input(const std::function<void (const input::InputManager&, double)> &func);

      /**
       * @brief Ajoute à la file d'affichage
       */
      void on_display(const std::function<void (render::Renderer*)> &func);
       ...
       * @param dt, delta time du moteur.
       */
      void process_input(const input::InputManager& input_manager, double dt) const;
       ...
       */
      void process_display(render::Renderer *renderer) const;

    private:
      std::vector<std::function<void ()>> setup_queue;
      std::vector<std::function<void (double)>> update_queue;
      std::vector<std::function<void (const input::InputManager&, double)>> input_queue;
      std::vector<std::function<void (render::Renderer*)>> rendering_queue;
  };
}
